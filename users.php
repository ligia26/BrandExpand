...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_user'])) {
        ...
        if ($stmt = $connection->prepare($check_email_sql)) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                // Prepare SQL query
                $sql = "INSERT INTO users (name, email, password, last_login, status) VALUES (?, ?, ?, NOW(), 1)";
...
$sql = "SELECT users.id, users.name, users.email, users.status FROM users";
